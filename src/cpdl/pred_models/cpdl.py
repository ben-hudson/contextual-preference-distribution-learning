...
class ContextualIO(BaseLightningModel):
    ...
    def get_prob(self, feats: torch.Tensor, n_samples: int):
        cost_dist = self.forward(feats)
        prob = self.smooth_fwd_opt(cost_dist, n_samples=n_samples).clamp(0, 1)
    ...
    def training_step(self, batch):
        ...
        choice_prob_pred, _ = self.get_prob(feats, n_samples=self.n_samples)
    ...
    def validation_step(self, batch, batch_idx, log_prefix: str = "val/"):
        ...
        choice_prob_pred, cost_dist_pred = self.get_prob(feats, n_samples=n_samples)
